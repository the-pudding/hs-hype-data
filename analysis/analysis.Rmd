...
export <- players %>% 
  mutate(top = ifelse(is.na(recruit_year), 0, 1),
         ...
         success = case_when(
           ...
           # allstars
           nba_mean_vorp >= 4 ~ "allstar",
           # great players
           between(nba_mean_vorp, 2, 4) ~ "great",
           # good players
           between(nba_mean_vorp, -0.5, 2) ~ "good",
           # bad players
           (rookie == 1 & is.na(nba_mean_vorp)) | nba_mean_vorp <= -0.5  ~ "bad",
           TRUE ~ NA_character_
         )) %>% 
  select(c(name, college, smallMult, rank, draft_year, draft_rd, draft_pk, recruit_year, nba_mean_vorp, top, highSchool, coll, draft, rookie, success))  %>% 
  mutate(val = 1) %>% 
