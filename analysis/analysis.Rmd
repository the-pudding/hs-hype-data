...
export <- players %>% 
  mutate(top = ifelse(is.na(recruit_year), 0, 1),
         nba_mean_vorp = round(nba_mean_vorp, 2),
         highSchool = case_when(
           # did they play beyond hs?
           college != "" | draft_year != "" ~ 1,
           TRUE ~ 0
         ),
         coll = case_when(
         ...
         success = case_when(
           ...
           # allstars
           nba_mean_vorp >= 5 ~ "allstar",
           # great players
           between(nba_mean_vorp, 0, 5) ~ "great",
           # good players
           between(nba_mean_vorp, -0.5, 0) ~ "good",
           # bad players
           nba_mean_vorp <= -0.5 ~ "bad",
           TRUE ~ NA_character_
         )) %>% 
  select(c(name, college, rank, draft_year, draft_rd, draft_pk, recruit_year, nba_mean_vorp, top, highSchool, coll, draft, rookie, success))  %>% 
  mutate(val = 1) %>% 
